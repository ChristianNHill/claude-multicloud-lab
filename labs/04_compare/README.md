...
The registry pairs each caller with the env vars it can't run without, so "is this configured?" is a data question rather than a try/except. Bedrock's list is empty because its credentials come from the AWS profile or instance metadata. There is no single variable to check, so the call is attempted and allowed to fail into the error path.
...
`rich` handles the table. The only judgment call is truncating the excerpt to 120 characters and printing the full text below the table. The table is for scanning the numbers. The section under it is for reading the answers.

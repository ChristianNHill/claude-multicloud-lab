...
### Step 3: tool results are feedback, not acknowledgements
...
Lab 03's `while True` is fine for a demo with one tool. An agent that decides its own next step needs a ceiling. A model that gets confused about whether it has finished will call `get_state` forever, and every iteration is a billed API call. The guard is three lines and it's the difference between a bug and an invoice.
...
5. Change `record_finding`'s return string to `"OK"` and re-run a few times. Watch the completion rate drop. That's how much work the feedback string was doing.
